...
const COMPACTION_INSTRUCTION = [
	...
	"- Preserve the latest interrupted action and whether its side effects are confirmed, unknown, or not started. Keep managed job IDs, log/artifact paths, last observed status, failed approaches, and the exact next verification. Never infer that a pending job completed or rerun it merely because the turn was interrupted.",
...
async function summarizeWithLlm(ctx, config, input, agent, signal) {
	...
	if (config.checkpointBudget !== void 0) {
		const budget = config.checkpointBudget;
		const info = await ctx.llm.resolveModelInfo(target.provider, target.model, signal);
		const capacity = Math.min(budget.contextWindow, info.context?.contextWindow ?? budget.contextWindow);
		const instructionTokens = ctx.tokenMeter.estimateMessage(messages[messages.length - 1]);
		const envelopeTokens = messages.reduce((sum, message) => sum + ctx.tokenMeter.estimateMessage(message), 0)
			+ Math.ceil(JSON.stringify(input.tools ?? []).length / 4) + 16;
		// The meter includes routed image pricing and provider usage calibration.
		const inputTokens = Math.max(envelopeTokens, ctx.tokenMeter.measure(agent.session).totalTokens + instructionTokens);
		options.maxTokens = Math.min(budget.summaryMaxTokens, config.maxTokens,
			Math.floor(capacity - inputTokens - budget.safetyTokens - 1));
		if (!Number.isFinite(options.maxTokens) || options.maxTokens < budget.summaryMinTokens) {
			throw new Error(`checkpoint summary has insufficient context headroom: input ~${inputTokens}, capacity ${capacity}, safety ${budget.safetyTokens}; history preserved`);
		}
	}
	...
	if (config.checkpointBudget !== void 0 && rawOutput.some((block) => block.type !== "text" && block.type !== "reasoning")) {
		throw new Error("checkpoint must contain text only; tool calls are never executed during summarization");
	}
...
async function compactSurfaceRegion(dependencies, session, start, end, agent, options, signal) {
	...
	try {
		...
		let savedSummary;
		if (options.checkpointBeforeReplace) {
			stage = "persistence";
			savedSummary = recordCompactionSummary(session, startEvent, summarized);
			await options.flush();
			signal?.throwIfAborted();
			assertStable(dependencies, session, summarized);
			options.onPhase?.("compacting");
		}
		stage = "commit";
		const pending = commitCompactionBody(session, startEvent, summarized, savedSummary);
		closing = true;
...
function recordCompactionSummary(session, startEvent, summarized) {
...
function commitCompactionBody(session, startEvent, summarized, savedSummary) {
	const { start, end, shadowedSeqs, shadowedTokenCount, summary, checkpointMessage } = summarized;
	const summaryEvent = savedSummary ?? recordCompactionSummary(session, startEvent, summarized);
...
var BasicCompactionEngine = class extends CompactionEngine {
	...
	*/
	async compactIfNeeded(agent, trigger, signal) {
		// This agent's guard owns pause -> checkpoint -> replace. Never prune or
		// run the older automatic path ahead of its reserved summary phase.
		if (agent[Symbol.for("dsh.contextGuard.checkpointPolicy.v1")]
			|| this.ctx.get("contextGuardPolicy")?.forAgent(agent)?.contextWindow !== void 0) return null;
		const target = routedTarget(agent.session);
	...
	*/
	checkpointNow(agent, signal, budget) {
		for (const key of ["contextWindow", "summaryMaxTokens", "summaryMinTokens", "safetyTokens"]) {
			if (!Number.isInteger(budget?.[key]) || budget[key] <= 0) throw new Error(`invalid checkpoint budget: ${key}`);
		}
		if (budget.summaryMinTokens > budget.summaryMaxTokens || budget.summaryMaxTokens + budget.safetyTokens >= budget.contextWindow) {
			throw new Error("invalid checkpoint reserve");
		}
		return this.compactNow(agent, signal, void 0, budget);
	}
	compactNow(agent, signal, sourceCommandId, checkpointBudget) {
		signal.throwIfAborted();
		try {
			return agent.runMaintenance(async (agentSignal) => {
				...
				try {
					...
					const first = systemHead(agent.session, nodes[0]) === void 0 ? 0 : 1;
					const range = checkpointBudget === void 0
						? selectCompactableRange(agent.session, this.ctx.tokenMeter.measure(agent.session), 0)
						: nodes.length <= first ? null : { start: nodes[first], end: nodes[nodes.length - 1] };
					if (range === null) return null;
					const dependencies = this.regionDependencies();
					if (checkpointBudget !== void 0) {
						dependencies.summarize = (input, owner, abort) => {
							checkpointBudget.onPhase?.("summarizing");
							const target = conversationTarget(owner);
							const policy = target === void 0 ? this.config : resolveTargetPolicy(this.config, target);
							return summarizeWithLlm(this.ctx, { ...policy, summarizationProvider: "", summarizationModel: "", checkpointBudget }, input, owner, abort);
						};
					}
					return await compactSurfaceRegion(dependencies, agent.session, range.start, range.end, agent, {
						owner: null,
						stability: "selected-span",
						checkpointBeforeReplace: checkpointBudget !== void 0,
						onPhase: checkpointBudget?.onPhase,
